  ...
  test("computeFeatureImportance, featureImportances") {
    /* Build tree for testing, with this structure:
      ...
      left2       parent
                left  right
     */
    val leftImp = new GiniCalculator(Array(3.0, 2.0, 1.0), 0, 3)
    val left = new LeafNode(0.0, leftImp.calculate(), leftImp)

    val rightImp = new GiniCalculator(Array(1.0, 2.0, 5.0), 0, 3)
    ...
    val left2Imp = new GiniCalculator(Array(1.0, 6.0, 1.0), 0, 3)
